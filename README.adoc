= Patched YubiHSM Shell 

== YubiHSM 2.7.2 opaque

This is a modified version of the YubiHSM Shell 2.7.2 that accepts values other than `Opaque object` for the `CKA_APPLICATION` attribute of CKO_DATA objects, when using the YubiHSM PKCS#11 library.

=== Root Cause Analysis

Two separate issues together cause custom values for CKA_APPLICATION to be ignored: one at write time and one at read time.

==== Issue #1 — `C_CreateObject` silently discards `CKA_APPLICATION`
...
==== Issue #2 — `get_attribute_opaque` always returns a hardcoded string
