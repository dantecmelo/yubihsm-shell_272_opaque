...
c// Length calculation — add this alongside the existing cka_label_pubkey term:
+ (meta_object->cka_application.len == 0
       ? 0
       : 3 + meta_object->cka_application.len)
c// Serialization — add after the existing write_meta_item calls:
...
C_CreateObject — parses CKA_APPLICATION into meta_object.cka_application, then calls write_meta_object which serializes it as a TLV tag 5 blob into a companion YH_OPAQUE object on the device, labelled "Meta object for 0x<seq><type><id>".
Login / populate_cache_with_data_opaques — loads all YH_ALGO_OPAQUE_DATA objects including both the data object and its meta companion into the slot cache. _get_object_desc detects the meta object via is_meta_object and calls read_meta_object, which deserializes TLV tag 5 back into meta_object.cka_application.
C_GetAttributeValue → get_attribute_opaque — find_meta_object_by_target locates the companion in cache; get_attribute_opaque reads meta_object->cka_application and returns your stored string instead of the hardcoded one.
